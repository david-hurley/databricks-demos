...
# Initialize Databricks clients and load data
...
df = read_ai_parse_results_table(sql_connection, os.getenv("CATALOG"), os.getenv("SCHEMA"), os.getenv("TABLE"))

# Initialize Dash app with Bootstrap and custom font
...
def get_error_alert(sql_conn, dataframe, catalog, schema, table):
    ...
    if sql_conn is None:
        return dbc.Alert("⚠️ Unable to establish SQL connection. Please check your SQL_WAREHOUSE_ID and credentials.", 
                        color="warning", style=ALERT_STYLE)
    if dataframe.empty:
        return dbc.Alert(f"⚠️ No data found in table {catalog}.{schema}.{table}. The table may not exist.", 
                        color="warning", style=ALERT_STYLE)
    return html.Div()

def prepare_grid_data(dataframe, search_value=None):
    """Convert dataframe to grid format and apply optional search filter."""
    if dataframe.empty:
        return []
    grid_data = []
...
# Set initial layout with grid data and any errors
error_alert = get_error_alert(sql_connection, df, os.getenv('CATALOG'), os.getenv('SCHEMA'), os.getenv('TABLE'))
...
def toggle_instructions_modal(open_clicks, close_clicks, is_open):
    """Toggle instructions modal on button click."""
    return not is_open
...
def update_grid_data(n_clicks, search_value, catalog, schema, table):
    """Refresh grid data when refresh button clicked or search value changes."""
    global df  # Update global dataframe when refreshing
    
    # Reload data from table if refresh button was clicked
    ...
    return prepare_grid_data(df, search_value), get_error_alert(sql_connection, df, catalog, schema, table)
...
def display_document(selected_rows, prev_clicks, next_clicks, dropdown_page, page_data):
    """Display selected document and handle page navigation."""
    current_page = page_data.get("current_page", 1) if page_data else 1
    total_pages = page_data.get("total_pages", 1) if page_data else 1
    
    # Determine which control triggered the callback and update page accordingly
    if callback_context.triggered:
        ...
        if trigger_id == "prev-page-btn":
            ...
            current_page = 1  # Reset to page 1 when selecting new document
    
    # Define button styles for enabled/disabled states
    DISABLED_STYLE = {**FONT_STYLE, "color": "#EEEDE9"}
    ENABLED_STYLE = {**FONT_STYLE, "color": "#0B2026"}
    
    # Default result when no document selected
    EMPTY_RESULT = (
        html.Div([html.P("← Select a document from the grid to view", 
                        style={"color": "#0B2026", "opacity": "0.6", "fontStyle": "italic", "padding": "40px", "textAlign": "center", "margin": "0"})]),
        "", True, True, DISABLED_STYLE, DISABLED_STYLE, [], 1,
        {"width": "120px", "fontFamily": "DM Sans", "color": "#EEEDE9"},
        {"current_page": 1, "total_pages": 1}
    )
    
    # Validate selection and row data
    ...
    # Parse document data to get total pages
    try:
    ...
    # Render document HTML for current page
    ...
    return (
        ...
        current_page,
        {"width": "120px", "fontFamily": "DM Sans", "color": "#0B2026"},
